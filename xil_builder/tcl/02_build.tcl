...
if { $::argc > 0 } {
  for {set i 0} {$i < $::argc} {incr i} {
    ...
    switch -regexp -- $option {
      "--prj" { incr i; set PRJ_NAME [lindex $::argv $i] }
      "--part" { incr i; set PART_NAME [lindex $::argv $i] }
      "--strat" { incr i; set STRATEGY [lindex $::argv $i] }
      default {
        if { [regexp {^-} $option] } {
...
if { ![file isdirectory $PRJ_DIR] } {
  exec mkdir -p $PRJ_DIR
}
set TOP_MODULE "$PRJ_NAME"

source ../tcl/01_synth_args.tcl 
source ../tcl/03_procedures.tcl
