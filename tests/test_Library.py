...
class TestLibrary(unittest.TestCase):
    ...
    def test_add_file_with_path_and_type(self):
        with patch.object(Path, 'is_file', return_value=True):
            self.library.add_file('/path/to/file', 'VHDL')
    ...
    def test_add_file_with_src_file(self):
        ...
        src_file.get_type.return_value = 'VHDL'
        src_file.get_path.return_value = Path('/path/to/file')
        with patch.object(Path, 'is_file', return_value=True):
            self.library.add_file(src_file)
            self.assertEqual(len(self.library.get_files()), 1)
